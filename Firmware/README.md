...
- watchdog самоподкармливается из прерывания — зависание задачи само по себе
  сброса не вызывает.

Дополнительно адаптер держит `hal_pwrmgr_lock(MOD_USR1)` только пока
энергизован тестовый режим (защита GPIO от sleep/wake). В «Норме» ядро спит.
...
3. Из GAP callbacks — `dpls_server_connected()`/`dpls_server_disconnected()`;
   из OSAL-таймера 1 с — `dpls_phy6252_tick()` (внутри `dpls_server_tick()`
   + сэмплирование ADC); из отдельного таймера —
...
журнала, смена имени/пароля, измерение напряжения на P20/P23. Каналы revision 2
на P15/P24 опрашиваются и передаются в приложение (на голом ките показывают
нули: делители не подключены), поэтому их точность и RGB на P7/P11/P18 требуют
проверки на новой плате.
...
кнопки на P34 — физический сброс требует перемычки P34↔GND; ставить её на P24
нельзя, это измерительный вход «+Т».
