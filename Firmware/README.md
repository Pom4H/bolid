...
- RX только через зашифрованную GATT characteristic, TX через
  indication/notification с пейсингом по ATT-подтверждению и таймаутом 2 с на
...
2. `dpls_gatt_add_service()` регистрируется, RX-callback идёт в
   ...
   `uint16`, notification гейтится по `ATT_GetCurrentMTUSize`).
...
(линия 1/31 — из архитектуры, VCAP — плейсхолдер `2000`), пороги источника
