...
`MOD_USR1` зарегистрирован hardware owner-ом и блокирует sleep только пока
активен ненормальный силовой режим (`OPEN_*` / `SHORT_*`). Обычное соединение
может спать; ADC не пересекает радиообмен благодаря post-connection-event gate.
...
прошивка запускает сразу после него ровно одно преобразование. Полный цикл
