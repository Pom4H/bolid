 ...
 *
 * Драйвер владеет только формой мигания; в idle он не требует таймера вообще. */
...
/* Возвращает задержку до следующего фронта. 0 означает, что LED idle и
 * вызывающий не должен держать периодический таймер. */
