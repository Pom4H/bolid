...
// GAP GATT Attributes
static uint8 attDeviceName[GAP_DEVICE_NAME_LEN] = "Test-DPLS-PB03F";
/*********************************************************************
    LOCAL FUNCTIONS
*/
...
void SimpleBLEPeripheral_Init( uint8 task_id )
{
    ...
    dpls_phy6252_init(simpleBLEPeripheral_TaskID);
...
uint16 SimpleBLEPeripheral_ProcessEvent( uint8 task_id, uint16 events )
{
    ...
    {
        dpls_phy6252_tick();
        osal_start_timerEx(simpleBLEPeripheral_TaskID, SBP_DPLS_TICK_EVT, 200);
...
static void peripheralStateNotificationCB( gaprole_States_t newState )
{
    ...
    {
        uint16 conn_handle = INVALID_CONNHANDLE;
        GAPRole_GetParameter(GAPROLE_CONNHANDLE, &conn_handle);
        dpls_phy6252_connected(conn_handle);
