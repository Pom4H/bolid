...
## Если «Служба Test-DPLS не найдена»

1. Плата должна быть прошита образом Test-DPLS и быть рядом с ноутбуком.
2. Параметры Windows → Bluetooth → удалите старое сопряжение с устройством (Test-DPLS-xxxx), затем в приложении «Повторить сопряжение».
3. Включите Bluetooth и разрешите приложениям доступ к нему.
4. Пересоберите и перезапустите клиент после `git pull`.
