...
- Сопряжение: CustomPairing ConfirmOnly + запись в зашифрованную RX-характеристику.
- ATT MTU Windows не отдаёт явно; лимит записи кадров принят 180 байт (достаточно для рукопожатия и журнала).

## Если «Служба Test-DPLS не найдена» / работает только после удаления сопряжения

Типичная рассинхронизация bond: Windows хранит старый LTK, а плата уже стёрла свой (перезагрузка / `ERASE_ALLBONDS`).

1. В приложении нажмите **«Повторить сопряжение»** — клиент сам вызывает `UnpairAsync` и заново pairing.
2. При первом сбое GATT/CCCD на уже paired-устройстве клиент тоже пробует один авто-сброс bond.
3. Если авто-сброс не помог: Параметры Windows → Bluetooth → удалите Test-DPLS вручную, подождите 2–3 с, повторите.
4. Плата должна быть прошита Test-DPLS и рядом; Bluetooth включён.
