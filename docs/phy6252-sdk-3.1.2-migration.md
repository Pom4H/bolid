...
Две ADC-правки применяются к общей версии `dpls_phy6252_app.c` проверяемым
скриптом `tools/prepare_phy6252_sdk312_app.py`. Скрипт требует ровно одно
совпадение для каждого изменения и восстанавливает исходник после сборки.
После слияния миграции этот механизм стал штатным переключателем сборки:
`DPLS_ADC=1` (по умолчанию) применяет правки и даёт образ с измерением,
`DPLS_ADC=0` собирает закоммиченный исходник без ADC — для голого PB-03F-Kit,
где висящие входы читаются как «низкий резерв» и защита ядра мгновенно снимает
тестовые режимы. CI собирает оба образа.
