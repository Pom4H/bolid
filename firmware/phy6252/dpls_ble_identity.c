...
/* Diagnostic branch: this file intentionally restores the exact BLE identity
 * ownership used by the real PB-03F with hardware-proven firmware 1.4.0.
 * Keep the rest of RC6 unchanged so the hardware test isolates this subsystem. */
...
{
    /* Diagnostic compatibility only: RC6 callers do not use this to own state. */
