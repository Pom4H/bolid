...
/* C99-compatible compile-time contract for hardware revision 2. Target builds
 * fail immediately if a board macro is changed without deliberately updating
 * the revision contract. */
#define DPLS_BOARD_ASSERT(name, expression) \
    typedef char dpls_board_assert_##name[(expression) ? 1 : -1]

DPLS_BOARD_ASSERT(iso_1_p31, DPLS_PIN_ISO_1 == GPIO_P31);
DPLS_BOARD_ASSERT(iso_2_p32, DPLS_PIN_ISO_2 == GPIO_P32);
DPLS_BOARD_ASSERT(iso_t_p33, DPLS_PIN_ISO_T == GPIO_P33);
DPLS_BOARD_ASSERT(kz_1_p14, DPLS_PIN_KZ_1 == GPIO_P14);
DPLS_BOARD_ASSERT(kz_2_p16, DPLS_PIN_KZ_2 == GPIO_P16);
DPLS_BOARD_ASSERT(kz_t_p17, DPLS_PIN_KZ_T == GPIO_P17);
DPLS_BOARD_ASSERT(port1_adc_p20, DPLS_PIN_PORT1_ADC == GPIO_P20);
DPLS_BOARD_ASSERT(port2_adc_p15, DPLS_PIN_PORT2_ADC == GPIO_P15);
DPLS_BOARD_ASSERT(port_t_adc_p24, DPLS_PIN_PORT_T_ADC == GPIO_P24);
DPLS_BOARD_ASSERT(vcap_adc_p23, DPLS_PIN_VCAP_ADC == GPIO_P23);
DPLS_BOARD_ASSERT(led_red_p07, DPLS_PIN_LED_RED == GPIO_P07);
DPLS_BOARD_ASSERT(led_green_p11, DPLS_PIN_LED_GREEN == GPIO_P11);
DPLS_BOARD_ASSERT(led_blue_p18, DPLS_PIN_LED_BLUE == GPIO_P18);
DPLS_BOARD_ASSERT(factory_reset_p34, DPLS_PIN_FACTORY_RESET == GPIO_P34);
DPLS_BOARD_ASSERT(reset_not_port_t, DPLS_PIN_FACTORY_RESET != DPLS_PIN_PORT_T_ADC);
DPLS_BOARD_ASSERT(green_not_port2, DPLS_PIN_LED_GREEN != DPLS_PIN_PORT2_ADC);

#undef DPLS_BOARD_ASSERT
