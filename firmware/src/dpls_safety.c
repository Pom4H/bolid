...
static dpls_safety_return_t input_failure(
...
dpls_safety_return_t dpls_safety_admission_reason(
    ...
    uint32_t now_ms)
{
    /* NORMAL is the sink state. Invalid input must never prevent returning safe. */
    ...
    return input_failure(in, now_ms);
}
    ...
    uint32_t now_ms)
{
    ...
    failure = input_failure(in, now_ms);
