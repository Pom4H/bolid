...
{
    dpls_safety_t s;
    dpls_safety_inputs_t in = healthy(1000u);
    dpls_safety_init(&s);
    assert(s.mode == DPLS_SAFE_NORMAL && s.revision == 1u);
    assert(dpls_safety_admission_reason(DPLS_SAFE_OPEN_T, &in, 1000u) == DPLS_SAFETY_RETURN_NONE);
...
static void test_admission_is_fail_safe(void)
{
    dpls_safety_inputs_t in = healthy(1000u);
    assert(dpls_safety_admission_reason(DPLS_SAFE_OPEN_T, &in, 1001u) == DPLS_SAFETY_RETURN_NONE);

    in.connected = false;
    assert(dpls_safety_admission_reason(DPLS_SAFE_OPEN_T, &in, 1001u) == DPLS_SAFETY_RETURN_DISCONNECT);
    in = healthy(1000u);
    in.measurements_ready = false;
    assert(dpls_safety_admission_reason(DPLS_SAFE_OPEN_T, &in, 1001u) == DPLS_SAFETY_RETURN_MEASUREMENT_LOST);
    in = healthy(1000u);
    in.real_short = true;
    assert(dpls_safety_admission_reason(DPLS_SAFE_OPEN_T, &in, 1001u) == DPLS_SAFETY_RETURN_REAL_SHORT);
    in = healthy(1000u);
    in.reserve_low = true;
    assert(dpls_safety_admission_reason(DPLS_SAFE_OPEN_T, &in, 1001u) == DPLS_SAFETY_RETURN_LOW_RESERVE);
    in = healthy(1000u);
    in.authenticated = false;
    assert(dpls_safety_admission_reason(DPLS_SAFE_OPEN_T, &in, 1001u) == DPLS_SAFETY_RETURN_SESSION_TIMEOUT);

    memset(&in, 0, sizeof(in));
    assert(dpls_safety_admission_reason(DPLS_SAFE_NORMAL, &in, 1001u) == DPLS_SAFETY_RETURN_NONE);
}
    ...
    for (mode = DPLS_SAFE_OPEN_T; mode <= DPLS_SAFE_SHORT_T; ++mode) {
        ...
        for (real_short = 0u; real_short <= 1u; ++real_short) {
            ...
            if (connected && authenticated && measured && !low && !real_short) {
                assert(dpls_safety_admission_reason((dpls_safety_mode_t)mode, &in, 1001u) ==
...
static void test_normal_is_always_fail_safe(void)
{
    ...
    for (mask = 0u; mask < 32u; ++mask) {
        ...
        assert(dpls_safety_admission_reason(DPLS_SAFE_NORMAL, &in, 1234u) == DPLS_SAFETY_RETURN_NONE);
...
int main(void)
{
    ...
    test_deadline_wrap();
    puts("dpls safety admission/return state-space tests passed");
    return 0;
}
