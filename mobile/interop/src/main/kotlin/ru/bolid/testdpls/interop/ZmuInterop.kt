...
private fun writeHeader(output: File) {
    ...
    println("generated ${encoded.size} Kotlin-wire request frames -> ${output.path}")
}
...
private fun verifyOutput(input: File) {
    ...
    check(command.payload[0].toInt() == 0)
    check(command.payload[1].toInt() == MODE_SHORT_1)
    check(readU16(command.payload, 2) == 300)

    val shortState = frames[8]
    check(shortState.payload[0].toInt() == MODE_SHORT_1)

    check(input.readText().contains("ZMU_E2E_OK")) { "ARM harness did not reach completion" }
    println("verified ${frames.size} ARM firmware responses with Kotlin wire decoder")
}

fun main(args: Array<String>) {
    require(args.size == 2) { "usage: generate|verify <path>" }
    when (args[0]) {
        "generate" -> writeHeader(File(args[1]))
        "verify" -> verifyOutput(File(args[1]))
