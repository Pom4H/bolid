...
/// Owns CoreBluetooth transport/lifecycle and coordinates the shared Test-DPLS session.
@MainActor
final class BleClient: NSObject, ObservableObject {
    ...
    private let session = DplsSessionBridge()
    private let messages = DplsMessageBridge()
    ...
    private var commandId: UInt32 {
        get { UInt32(truncatingIfNeeded: session.commandId) }
        set { session.commandId = Int64(newValue) }
    }
    private var sessionId: UInt32 {
        get { UInt32(truncatingIfNeeded: session.sessionId) }
        set { session.sessionId = Int64(newValue) }
    }
    private var sessionToken: Data {
        get { Data(dplsCoreHex: session.sessionTokenHex) ?? Data(count: 8) }
        set { session.sessionTokenHex = newValue.dplsCoreHex }
    }
    private var clientNonce: Data {
        get { Data(dplsCoreHex: session.clientNonceHex) ?? Data(count: 16) }
        set { session.clientNonceHex = newValue.dplsCoreHex }
    }
    private var deviceNonce: Data {
        get { Data(dplsCoreHex: session.deviceNonceHex) ?? Data(count: 16) }
        set { session.deviceNonceHex = newValue.dplsCoreHex }
    }
    private var authSalt: Data {
        get { Data(dplsCoreHex: session.authSaltHex) ?? Data(count: 16) }
        set { session.authSaltHex = newValue.dplsCoreHex }
    }
    private var initialized: Bool {
        get { session.initialized }
        set { session.initialized = newValue }
    }
    private var reachedReady: Bool {
        get { session.reachedReady }
        set { session.reachedReady = newValue }
    }
    private var awaitingDeviceInfo: Bool {
        get { session.awaitingDeviceInfo }
        set { session.awaitingDeviceInfo = newValue }
    }
    private var legacyFirmware: Bool {
        get { session.legacyFirmware }
        set { session.legacyFirmware = newValue }
    }
    ...
    private func parseDeviceInfo(_ raw: Data) {
        awaitingDeviceInfo = false
        guard let parsed = messages.parseDeviceInfoHex(payloadHex: raw.dplsCoreHex) else { return }
        let info = DeviceInfo(
            deviceId: UInt32(truncatingIfNeeded: parsed.deviceId),
            protocolVersion: Int(parsed.protocolVersion),
            firmwareVersion: parsed.firmwareVersion,
            hardwareRevision: Int(parsed.hardwareRevision),
            adcPresent: parsed.adcPresent,
            hardwareReadback: parsed.hardwareReadback,
            adcCalibrated: parsed.adcCalibrated,
            multiVoltageReport: parsed.multiVoltageReport,
            userName: parsed.userName
        )
        uiState.deviceInfo = info
        if var selected = uiState.selectedDevice {
            selected.userName = info.userName.isEmpty ? selected.userName : info.userName
    ...
    private func parseState(_ payload: Data) {
        ...
        guard let parsed = messages.parseStateHex(payloadHex: payload.dplsCoreHex, nowMillis: nowMs),
              let mode = DplsMode.fromWire(Int(parsed.mode))
        else { return fail("Повреждённый STATE_REPORT") }
        let uptimeSeconds = UInt32(truncatingIfNeeded: parsed.uptimeSeconds)
        uiState.phase = .ready
        uiState.statusText = "Состояние получено"
        uiState.state = DeviceState(
            mode: mode,
            voltageMv: Int(parsed.voltageMv),
            powerSource: parsed.powerSource == 0 ? .dpls : .reserve,
            reserveLow: parsed.reserveLow,
            realShort: parsed.realShort,
            automaticReturnSeconds: Int(parsed.automaticReturnSeconds),
            uptimeSeconds: uptimeSeconds,
            revision: UInt32(truncatingIfNeeded: parsed.revision),
            receivedAtMillis: parsed.receivedAtMillis,
            lineVoltageValid: parsed.lineVoltageValid,
            reserveValid: parsed.reserveValid,
            powerValid: parsed.powerValid,
            autoIsoValid: parsed.autoIsoValid,
            adcCalibrated: parsed.adcCalibrated,
            port1VoltageMv: Int(parsed.port1VoltageMv),
            port2VoltageMv: Int(parsed.port2VoltageMv),
            portTVoltageMv: Int(parsed.portTVoltageMv),
            reserveVoltageMv: Int(parsed.reserveVoltageMv),
            port1VoltageValid: parsed.port1VoltageValid,
            port2VoltageValid: parsed.port2VoltageValid,
            portTVoltageValid: parsed.portTVoltageValid,
            reserveVoltageValid: parsed.reserveVoltageValid
        )
        uiState.deviceBootEpochSeconds = nowMs / 1000 - parsed.uptimeSeconds
        uiState.authenticated = true
        uiState.identifyActive = false
        uiState.identifyLedLive = false
        uiState.commandInProgress = false
    ...
    private func finishLog() {
        ...
        var offset = 0
        while offset + 10 <= logBytes.count {
            let raw = logBytes.subdata(in: offset ..< offset + 10)
            offset += 10
            guard let parsed = messages.parseEventHex(recordHex: raw.dplsCoreHex) else { continue }
            records.append(
                EventRecord(
                    sequence: UInt32(truncatingIfNeeded: parsed.sequence),
                    timestampSeconds: UInt32(truncatingIfNeeded: parsed.timestampSeconds),
                    type: Int(parsed.type),
                    parameter: Int(parsed.parameter)
                )
            )
        }
    ...
    private func authenticatedPayload() -> Data {
        Data(dplsCoreHex: session.authenticatedPayloadHex) ?? Data()
    }
    ...
    private func nextSequence() -> UInt16 {
        UInt16(truncatingIfNeeded: session.nextSequence())
    }
    ...
    private func commandRejectReason(_ status: Int) -> String {
        messages.commandRejectReason(status: Int32(status))
    }
}
...
private extension Data {
    var dplsCoreHex: String {
        map { String(format: "%02x", $0) }.joined()
    }

    init?(dplsCoreHex: String) {
        guard dplsCoreHex.count.isMultiple(of: 2) else { return nil }
        self.init(capacity: dplsCoreHex.count / 2)
        var index = dplsCoreHex.startIndex
        while index < dplsCoreHex.endIndex {
            let next = dplsCoreHex.index(index, offsetBy: 2)
            guard let byte = UInt8(dplsCoreHex[index ..< next], radix: 16) else { return nil }
            append(byte)
            index = next
        }
    }
