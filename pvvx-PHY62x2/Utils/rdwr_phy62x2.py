...
class phyflasher:
	...
	def send_blk(self, stream, offset, size, blkcnt, blknum, segment = 0):
		...
		# PHY6222 ROM rejects final cpbin chunks between 0x1001 and 0x1fff
		# bytes. Pad such tails with erased-flash bytes to a full transfer block.
		if size > 0x1000 and size < PHY_WR_BLK_SIZE:
			data += b'\xff' * (PHY_WR_BLK_SIZE - size)
			size = PHY_WR_BLK_SIZE
		for e in data:
			if e != 0xff:
				self._port.timeout = 1
	...
	def WriteBlockFlash(self, stream, offset = 0, size = 0x8000, segment = PHY_SRAM_ADDR):
		...
		if self.autoerase:
			erase_size = size
			tail = size % PHY_WR_BLK_SIZE
			if tail > 0x1000:
				erase_size += PHY_WR_BLK_SIZE - tail
			if not self.EraseSectorsFlash(offset, erase_size):	
				return False
