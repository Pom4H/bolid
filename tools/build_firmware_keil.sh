#!/usr/bin/env bash
# Build Test-DPLS with the full pinned PHY62XX SDK 3.1.2 and Keil MDK
# Arm Compiler 6, then combine the load regions into a flashable Intel HEX.
#
...
for tool in cbuild fromelf python3; do
    if ! command -v "$tool" >/dev/null 2>&1; then
        echo "$tool not found. Activate the Keil MDK / Arm Compiler 6 environment from:" >&2
