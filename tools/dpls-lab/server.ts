...
function phoneAsset(pathname: string): Response | null {
  ...
  const rel =
    pathname === "/" || pathname === "/index.html" || pathname === "/phone" || pathname === "/phone/"
      ? "index.html"
      : pathname.replace(/^\/phone\//, "/").replace(/^\//, "");
...
console.log(`Test-DPLS http://127.0.0.1:${PORT}${PHONE_DIST === undefined ? " (wasm not built)" : ""}`);
